...
-(void) findLocationWithLat:(CGFloat) lat andLong:(CGFloat)lng
{
	// store the query
	self.searchQuery = [NSString stringWithFormat:@"latlong://%f,%f", lat, lng];
...
-(void)startGeocoding
{
	...
	else if(version == 3)
	{
        ...
        if ([searchQuery rangeOfString:@"latlong://"].location != NSNotFound)
        {
		// Create the url to Googles geocoding API, we want the response to be in XML
            mapsUrl = [[NSString alloc] initWithFormat:@"http://maps.google.com/maps/api/geocode/xml?latlng=%@&sensor=false", 
							 [searchQuery stringByReplacingOccurrencesOfString:@"latlong://" withString:@""]];
        }
        else
        {
            mapsUrl = [[NSString alloc] initWithFormat:@"http://maps.google.com/maps/api/geocode/xml?address=%@&sensor=false", 
                       searchQuery];
        }
	...
	DLog(@"Found placemarks: %d", [self.results count]);
